...
func (host *nodeLanguageHost) constructConfigSecretKeys(req *pulumirpc.RunRequest) (string, error) {
	secretConfigKeys := req.GetSecretConfigKeys()
	if secretConfigKeys == nil {
		return "[]", nil
	}

	secretConfigKeysJSON, err := json.Marshal(secretConfigKeys)
	if err != nil {
		return "", err
	}

	return string(secretConfigKeysJSON), nil
}
