...
func (src *evalSource) Iterate(
	...
	secretConfigKeys := src.runinfo.Target.Config.SecureKeys()
	...
	iter.forkRun(opts, config, secretConfigKeys)
...
func (iter *evalSourceIterator) forkRun(opts Options, config map[config.Key]string, secretConfigKeys []config.Key) {
